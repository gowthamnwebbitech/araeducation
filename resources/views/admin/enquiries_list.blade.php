            ...
            <div class="card-body">
                <div class="table-responsive" style="overflow-x: auto;">
                    <table id="example4" class="display table table-bordered verticle-middle table-striped table-responsive-sm" style="min-width: 845px;">
                        ...
                        <tbody>
                            @php $contact = App\Models\Enquiry::latest()->get(); @endphp
                            @foreach($contact as $key => $item)
                            <tr>
                                <td>{{ $key+1 }}</td>
                                <td>{{ $item->name }}</td>
                                <td>{{ $item->email }}<br>({{ $item->phone }})</td>
                                <td data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $item->message }}">
                                    {{ \Illuminate\Support\Str::limit($item->message, 30) }}
                                </td>
                                <td>
                                    <div class="d-flex">
                                        <a href="javascript:void(0);" class="btn btn-danger shadow btn-xs sharp confirm-delete" data-href="{{route('enquiry.destroy', $item->id)}}" title="Delete">
