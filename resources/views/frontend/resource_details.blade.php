...
@section('title', $resource_details->meta_title ?? 'CS ACS CA CMA  Institute Coimbatore ACCA USCMA CLAT Coaching center Tamilnadu - ARA Education')
@section('description', $resource_details->meta_description ?? 'Coimbatore’s best Coaching Institute for CA CS ACS CSEET CMA ICWA academy  is ARA Education. Tamilnadu’s No1 Coaching Center for  ACCA USCMA CLAT training India')
@section('keyword', $resource_details->meta_keyword ?? 'ca institute in Coimbatore ara education,best institute for ca
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-11 col-md-11">
                ...
                <h2 class="blog-title">{{ $resource_details->title }}</h2>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('resources') }}">Resource</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ $resource_details->title }}</li>
                    </ol>
                </nav>
                <div class="row gx-5 mt-4">
                    <div class="col-lg-6">
                        <div class="blog-img">
                            <img src="<?php echo url(''); ?>/public/resource_images/{{ $resource_details->image }}" alt="">
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <p class="blog-text">{!! $resource_details->description !!}</p>
                        @if($resource_details->pdf_file)
                            <a href="{{ url('public/resource_files/'.$resource_details->pdf_file) }}" style="margin-top: 10px;margin-left: 5px;font-weight: 700;text-decoration: underline;" download><i class="bi bi-file-earmark-pdf"></i> Download PDF</a>
                        @endif
                    </div>
